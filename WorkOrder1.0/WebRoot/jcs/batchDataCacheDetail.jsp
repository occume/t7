<%@ page import="java.util.Set,java.util.Date,java.text.SimpleDateFormat,java.util.HashMap,java.util.List,java.util.ArrayList,java.util.Iterator,org.apache.jcs.admin.*" contentType="text/html;charset=utf-8"%>
		  	 ...
		  	 <%
				...
				if(intPageCount>0){//将记录指针定位到待显示页的第一条记录上
	  			...
	  			while(i<intPageSize && it.hasNext()){
					...
					// LOGGER.debug("根据工单号从数据库获取工单的详细信息：accountname="+wo.getUserInfo().getAccountname()
					//+"|问题类型："+wo.getUserInfo().getLvlTwo().getName()+"|创建时间："+wo.getCreateTime()+"|服务器信息："+(wo.getUserInfo().getServer().getName()+wo.getUserInfo().getServer().getArea())
					//+"|问题描述："+wo.getUserInfo().getMemo());			  		
				   %>  
			   		<tr>
				   	 	 <td><a href="<%=request.getContextPath()%>/jcs/batchDataCache.jsp?
				   	 	 				batchDataCacheKey=<%=elementInfo.getKey()%>
				   	 	 				&accountname=<%=batchData.getWorkOrder().getUserInfo().getAccountname()%>
				   	 	 				&lvlTwo=服务器故障
				   	 	 				&createTime=<%=batchData.getWorkOrder().getCreateTime() %>
				   	 	 				&server=<%=batchData.getWorkOrder().getUserInfo().getServer().getName() %>
				   	 	 				&memo=<%=batchData.getWorkOrder().getUserInfo().getMemo() %>"><%=elementInfo.getKey()%></a></td>
