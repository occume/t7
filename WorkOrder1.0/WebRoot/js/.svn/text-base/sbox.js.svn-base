$(function(){
	...
	$(window).scroll(function(){
		...
		(function(win,doc){
			var visi = !1, $wrap, $win,
			winWidth, winHeight, wrapWidth, wrapHeight;
			var SBOX = function(){
				return {
					init:function(){
						$win = $(win), $wrap = $("#wrap"),
						winWidth = $win.width(), winHeight = $win.height(),
						wrapWidth = $wrap.width(), wrapHeight = $wrap.height();
						var scrTop = $win.scrollTop();	
						$wrap.css({left:(winWidth - wrapWidth)/2, 
									top:(scrTop + winHeight - 80)});
						$("#help").css({left:0,top:-205,display:"none"});
						var ll = $wrap.offset().left - $("#im").width(),
							tt = $wrap.offset().top;
						//alert(ll);
						$("#im").css({left:ll,top:tt,display:"none"});
						$wrap.css({display:"none"});
						//this.bind();
					},
					replace:function(){
						var scrTop = $win.scrollTop();
						$wrap.css({top: scrTop + winHeight - 80});
		...
		function showIM( data ){
			...
			$("#im").fadeIn();
			var p = $("<p>"+ data.id + ":" + data.msg + "</p>");
			
			$("#im").append(p);
			var name = $("#welcome").text().substring(4);
			if(name == data.id){
				p.css({color:colors[0]});
			}else{
				p.css({color:colors[1]});
			}
