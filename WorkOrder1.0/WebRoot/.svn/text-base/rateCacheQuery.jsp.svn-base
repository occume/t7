...
<html>
  ...
  <body>
    	...
    	<h4><a href="/rateCacheQuery.jsp?deleteId=all">一键解锁</a></h4>
    			<table border="1" cellpadding="0" cellspacing="0">
    				...
    				<c:forEach items="${CacheElementInfo}" var="obj" varStatus="index">
    					...
    					<td>${obj.key}</td>
    					<td>${obj.value}</td>
    					<td><a href="/rateCacheQuery.jsp?deleteId=${obj.key}&currentPage=${currentPage}">删除</a></td>
    					<td></td>
    				</tr>
    				</c:forEach>
    				<tr>
    					<td>
    							<c:if test="${currentPage>1}">
    								<a href="/rateCacheQuery.jsp?currentPage=1">首页</a>
    							</c:if> 
    							<c:if test="${currentPage<=1}">
    								第一页
    							</c:if>
    					...
    					<td>
    							<c:if test="${currentPage>1}">
    								<a href="/rateCacheQuery.jsp?currentPage=${currentPage-1}">上一页</a>
    							</c:if> 
    							<c:if test="${currentPage<=1}">
    								上一页
    							</c:if>
    					...
    					<td>
    							<c:if test="${currentPage<totalPage}">
    								<a href="/rateCacheQuery.jsp?currentPage=${currentPage+1}">下一页</a>
    							</c:if> 
    							<c:if test="${currentPage>=totalPage}">
    								下一页
    							</c:if>
    					...
    					<td>
    							<c:if test="${currentPage<totalPage}">
    								<a href="/rateCacheQuery.jsp?currentPage=${totalPage}">尾页</a>
    							</c:if> 
    							<c:if test="${currentPage>=totalPage}">
    								尾页
    							</c:if>
